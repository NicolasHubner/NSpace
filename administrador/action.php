	...
	if ($_GET['model'] == 'perfil' && ($_GET['action'] == 'cadastro'
		...
		try {
			...
			if (isset($_GET['action']) && $_GET['action'] != 'listar'){
				...
				->addWhere('p.tipo = 1 OR p.tipo = 2 OR p.tipo = 3 OR p.tipo = 4 OR p.tipo = 5 OR p.tipo = 6 OR p.tipo = 7 OR p.tipo = 8 OR p.tipo = 9 OR p.tipo = 10 OR p.tipo = 11 OR p.tipo = 12')
			...
			// Tratamento de retorno
			$_SESSION['return_type'] 	= 'error';
			$_SESSION['return_message']	= 'Ocorreu um erro de permissão, tente novamente.';
			echo '
			<div class="'.$_SESSION['return_type'].' grid_12">
			<h3>'.$_SESSION['return_message'].'</h3>
			<a href="#" class="hide_btn">&nbsp;</a>
