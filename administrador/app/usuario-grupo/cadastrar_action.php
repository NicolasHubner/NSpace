...
try {
	...
	$retUsuarioGrupo	= 	Doctrine_Query::create()->select()->from('UsuarioGrupo')
					->where('dns LIKE ?', $_POST['dns'])->execute();
	
	if ($retUsuarioGrupo->count() > 0){
		...
		$objUsuarioGrupo->nome 	= $_POST['nome'];
		$objUsuarioGrupo->status	= $_POST['status'];
		$objUsuarioGrupo->dns		= $_POST['dns'];
		$objUsuarioGrupo->save();
