...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Anuncio a')->leftJoin('a.Estado e')->leftJoin('a.Cidade ci')->leftJoin('a.Categoria c')->leftJoin('a.Status s')->leftJoin('a.Plano pl')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Anuncio a')->leftJoin('a.Estado e')->leftJoin('a.Cidade ci')->leftJoin('a.Categoria c')->leftJoin('a.Status s')->leftJoin('a.Plano pl')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			if (isset($value['imagem'])&&$value['imagem']!='') {
				$value['imagem'] = '<center><img class="img-admin-resp" src="'.URL_ANUNCIO.$value['imagem'].'"></center>';
			}

			if (isset($value['pagamento'])&&$value['pagamento']==1) {
				$Status = 'Aprovado';
				$value['pagamento'] = $Status;
			} else if (isset($value['pagamento'])&&$value['pagamento']==2) {
				$Status = 'Pagamento negado';
				$value['pagamento'] = 'Pagamento negado';
			} else {
				$Status = 'Aguardando';
				$value['pagamento'] = 'Aguardando';
			}

			if (isset($value['Status']['nome'])&&$value['Status']['nome']!='') {
				$value['Status']['nome'] = $value['Status']['nome'].'<a style="color: #5a5a5a;" href="" data-toggle="tooltip" data-original-title="'.$value['Plano']['nome'].'"><i class="fas fa-shield"></i></a>';
			}
