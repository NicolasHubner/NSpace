	...
	<div class="col-md-12">
        <div class="block-flat">
			...
			try {
				...
				// Seleciona os dados
				$res = Doctrine_Core::getTable('Depoimento')->find($_GET['id']);
			...
			<form class="form" action="<?php echo URL_ADMIN.'action/'.$_GET['model'].'/'.$_GET['action'].'/'; ?>" method="post" id="formPerfil" enctype="multipart/form-data">
				...
				<div class="row">
					<div class="col-md-12">
						<label>Texto:</label>
						<input type="text" name="texto" class="form-style validate[required]" value="<?php echo $res->texto; ?>" />
					</div>
				</div>

				<?php
				if (isset($res->imagem)&&$res->imagem!='') {
					$imagem = URL_DEPOIMENTO.$res->imagem;
				} else {
