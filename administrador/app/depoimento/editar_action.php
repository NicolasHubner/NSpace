...
try {
	...
	// Update
	$objDepoimento						= Doctrine_Core::getTable('Depoimento')->find($_POST['id']);
	$objDepoimento->nome 					= $_POST['nome'];
	$objDepoimento->status 				= $_POST['status'];
	$objDepoimento->destaque 				= $_POST['destaque'];
	$objDepoimento->texto 				= $_POST['texto'];
	$objDepoimento->cargo 				= $_POST['cargo'];
	$objDepoimento->save();

	if (isset($_FILES['imagem']) && $_FILES['imagem']['error'] == 0){
		$fileType = Util::checkImageType($_FILES['imagem']['type']);
		if ($fileType){
			$fileName = $_POST['dns'].'-'.$objDepoimento->id;

			$imgPath = PATH_DEPOIMENTO.$fileName.'.'.$fileType;
			...
			if ($fileName){
				$objDepoimento->imagem = $fileName.'.'.$fileType;
				$objDepoimento->save();
			}
		}
	}
