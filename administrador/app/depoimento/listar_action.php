...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Depoimento d')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Depoimento d')
					->where($where)->orderby($orderby)->offset($_GET['iDisplayStart'])
