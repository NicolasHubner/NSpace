...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Equipe e')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Equipe e')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			if (isset($value['imagem'])&&$value['imagem']!='') {
				$value['imagem'] = '<center><img class="img-admin-resp" src="'.URL_EQUIPE.$value['imagem'].'"></center>';
			}
