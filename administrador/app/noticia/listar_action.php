...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Noticia n')->leftJoin('n.NoticiaCategoria nc')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Noticia n')->leftJoin('n.NoticiaCategoria nc')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			if (isset($value['imagem'])&&$value['imagem']!='') {
				$value['imagem'] = '<center><img class="img-admin-resp" src="'.URL_NOTICIA.$value['imagem'].'"></center>';
			}
