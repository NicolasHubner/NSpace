...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Blog b')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Blog b')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			if (isset($value['imagem'])&&$value['imagem']!='') {
				$value['imagem'] = '<center><img class="img-admin-resp" src="'.URL_BLOG.$value['imagem'].'"></center>';
			}
