...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Banner')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Banner')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			$value['action'] .= $action;
         	...
         	if($value['imagem']!='')
				$value['imagem'] = '<img src="'.URL_BANNER.$value['imagem'].'" height=35px>';
			else
				$value['imagem'] = "";
