...
try {
	...
	$where .= ' AND plano_id = '.$_GET['plano_id'];

	
	// Buscas (Geral e Paginada)
	$retAll		= 	Doctrine_Query::create()->select()->from('Beneficio b')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Beneficio b')
					->where($where)
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			$value['action'] .= $action;
			$value['ordem'] = "<input type='text' name='ordem' codigo='".$value['id']."' style='width: 45px' class='ordem form-control validate[custom[onlyNumberSp],min[0],max[99999]]' value='".$value['ordem']."'>";
