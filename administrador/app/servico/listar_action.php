...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Servico s')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Servico s')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			$value['destaque'] = $_STATUS[$value['destaque']];
			$value['data_cadastro']	=	date('d/m/Y H:i', strtotime($value['data_cadastro']));
			$value['ordem'] = "<input type='text' name='ordem' codigo='".$value['id']."' style='width: 100%' class='ordem form-style validate[custom[onlyNumberSp],min[0],max[99999]]' value='".$value['ordem']."'>";

			if (isset($value['imagem'])&&$value['imagem']!='') {
				$value['imagem'] = '<center><img class="img-admin-resp" src="'.URL_SERVICO.$value['imagem'].'"></center>';
			}
