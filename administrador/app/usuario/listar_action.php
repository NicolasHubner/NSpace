...
try {
	...
	$where = $where==''?'id <> 1 AND id <> 2':$where.' AND id <> 1 AND id <> 2';
	
	// Buscas (Geral e Paginada)
	$retAll		= 	Doctrine_Query::create()->select()->from('Usuario u')
					->leftJoin('u.UsuarioGrupo ug')->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Usuario u')
					->leftJoin('u.UsuarioGrupo ug')->where($where)->offset($_GET['iDisplayStart'])
