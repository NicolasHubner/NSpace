...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Galeria')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Galeria')
   ...
   //       	if($value['foto']!='')
			// 	$value['foto'] = '<img src="'.URL_BANNER."/".$value['foto'].'" height=35px>';
			// else
			// 	$value['foto'] = "";
