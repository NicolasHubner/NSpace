...
try {
	...
	// print_r($_GET['verificado']);

	$where .= isset($_GET['verificado'])&&$_GET['verificado']!=''?" and verificado = ".$_GET['verificado']:'';
	
	// Buscas (Geral e Paginada)
	$retAll		= 	Doctrine_Query::create()->select()->from('Cliente cl')->leftJoin('cl.Estado es')->leftJoin('cl.Cidade c')->leftJoin('cl.TipoCliente tc')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Cliente cl')->leftJoin('cl.Estado es')->leftJoin('cl.Cidade c')->leftJoin('cl.TipoCliente tc')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			$value['status'] = $_STATUS[$value['status']];
			$value['data_cadastro'] = date('d/m/Y H:i:s', strtotime($value['data_cadastro']));

			if (isset($value['verificado'])&&$value['verificado']==2) {
				$value['TipoCliente']['nome'] = $value['TipoCliente']['nome'].'<span style="font-weight: 600;
			    color: #03a9f4;
			    font-size: 16px;
			    margin-left: 7px;" data-toggle="tooltip" data-original-title="Conta verificada"><o class="fal fa-badge-check"></i><span>'; 
			} 
