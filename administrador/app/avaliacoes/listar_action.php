...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('ReservaAvaliacao ra')->leftJoin('ra.Cliente c')->leftJoin('ra.Reserva r')->leftJoin('ra.Anuncio a')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('ReservaAvaliacao ra')->leftJoin('ra.Cliente c')->leftJoin('ra.Reserva r')->leftJoin('ra.Anuncio a')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			if (isset($value['Reserva']['id'])&&$value['Reserva']['id']!='') {
				$value['Reserva']['id'] = '#'.$value['Reserva']['id'];
			}

            if (isset($value['avaliacao'])&&$value['avaliacao']==1) {
                $value['avaliacao'] = '<i class="fas fa-star"></i>
