...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Cancelamento c')->leftJoin('c.Plano p')->leftJoin('c.Anuncio a')->leftJoin('c.Cliente cl')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Cancelamento c')->leftJoin('c.Plano p')->leftJoin('c.Anuncio a')->leftJoin('c.Cliente cl')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			$value['data_cancelamento']	=	date('d/m/Y H:i', strtotime($value['data_cancelamento']));
			$value['status'] = isset($value['status'])&&$value['status']==1?'Aguardando':'Cancelado';
