...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Arquivo')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Arquivo')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			// Destaque
			$value['data_cadastro'] = isset($value['data_cadastro'])&&$value['data_cadastro']!='0000-00-00 00:00:00'?date('d/m/Y H:i:s', strtotime($value['data_cadastro'])):'-';

			// Principal
			$value['url'] = isset($value['arquivo'])?URL_ARQUIVO.$value['arquivo']:'-';
