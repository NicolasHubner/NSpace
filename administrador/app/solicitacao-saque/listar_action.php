...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('SolicitacaoSaque sa')->leftJoin('sa.Cliente c')->leftJoin('sa.TipoTransacao ts')->leftJoin('sa.StatusSaque s')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('SolicitacaoSaque sa')->leftJoin('sa.Cliente c')->leftJoin('sa.TipoTransacao ts')->leftJoin('sa.StatusSaque s')
