...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Reserva r')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Reserva r')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			$value['valor_cliente'] = 'R$'.number_format($value['valor_cliente'], 2, ',', '.');
			$value['valor_nspace'] = 'R$'.number_format($value['valor_nspace'], 2, ',', '.');
			$value['valor_afiliado'] = 'R$'.number_format($value['valor_afiliado'], 2, ',', '.');
