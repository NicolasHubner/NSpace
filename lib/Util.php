...
abstract class Util {
	...
	static function formata_valor (&$valor) {
		$valor 				= isset($valor)&&$valor!=''?$valor:'0';
	...
	/**
	 * Calcular Frete
	 * @return array prazo de entrega, valor do frete
	 */
	static function calcFrete($tipo, $produto_id, $qtde, $cep_param=null){
        $frete = new RsCorreios();
        
        $objConfiguracao = Doctrine_Core::getTable('Configuracao')->find(1);
        $objProduto = Doctrine_Core::getTable('Produto')->find($produto_id);
        ...
        $resposta = $frete
            ->setCepOrigem($objConfiguracao->cep_origem)
            ->setCepDestino($cep)
            ->setLargura($objProduto->largura)
            ->setComprimento($objProduto->comprimento)
            ->setAltura($objProduto->altura)
            ->setPeso($objProduto->peso*$qtde)
            ->setFormatoDaEncomenda(RsCorreios::FORMATO_CAIXA)
            ->setServico(empty($tipo) ? RsCorreios::TIPO_PAC : $tipo)
            ->dados();

        // print_r($resposta);
        if(isset($resposta['valor'])&&$resposta['valor']>0){
	        $returnArray['prazo_entrega'] = isset($resposta['prazoEntrega'])&&$resposta['prazoEntrega']>0?$resposta['prazoEntrega']:$prazo_entrega; 
	        $returnArray['valor_frete'] = isset($resposta['valor'])&&$resposta['valor']>0?$resposta['valor']:0;
	    }else{
	    	return false;
	    }
    ...
    static function uploadImage2($pathFile, $pathDestination, $ext, $name = '',
	...
	static function calculaFrete($origem, $destino, $peso, $valor = 0.00){
		try {
			
			$res = file_get_contents('http://developers.agenciaideias.com.br/correios/frete/json/'.$origem.'/'.$destino.'/'.$peso.'/'.number_format($valor, 2, '.', ''));

			$valor = json_decode($res);
