	...
	$(".range-slider-ui").each(function () {
        ...
        $(this).slider({
            ...
            slide: function (event, ui) {
                ...
                $(this).children(".current-min").val(currentMin.toFixed(2).replace(".",","));
                $(this).children(".current-max").val(currentMax.toFixed(2).replace(".",","));
            }
        });

        var currentMin = parseInt($(this).slider("values", 0), 10);
        var currentMax = parseInt($(this).slider("values", 1), 10);
        $(this).children(".min-value").text(currentMin.toFixed(2).replace(".",","));
        $(this).children(".max-value").text(currentMax.toLocaleString("pt-BR", { style: "currency" , currency:"BRL"}));
        $(this).children(".current-min").val(currentMin.toFixed(2).replace(".",","));
        $(this).children(".current-max").val(currentMax.toFixed(2).replace(".",","));
    });
